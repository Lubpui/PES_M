...
# --------------------------------
# 0) Coordinate Scaling Constants
# --------------------------------
# เครื่องชุดนี้ความละเอียด 1280x720 (ใหญ่กว่า 960x540 ของ PES_BOT 1.3333 เท่า)
...
try:
    # Ensure UTF-8 is used for console output on Windows
    stream_handler.setStream(io.TextIOWrapper(stream_handler.stream.buffer, encoding='utf-8', errors='replace', line_buffering=True))
except Exception:
    # Fallback: leave default stream if rewrapping fails
    pass
...
# ✅ Phase 1: Global Login Queue Lock
# ป้องกัน concurrent login ของ 20+ devices
...
def process_device_start_queue():
    ...
    if app and app.winfo_exists():
        ...
        delayDevice = 0 if selected_connect_mode == 'รีปกติ' else 2000  # ดอง delay น้อยกว่า ฟาร์ม
...
def process_device_reset_queue():
    ...
    if app and app.winfo_exists():
        ...
        delayDevice = 500 if selected_connect_mode == 'รีปกติ' else 800  # ดอง delay มากขึ้นเพื่อ batch processing
...
def connect_devices(re_reroll_file_path):
    ...
    try:    
        ...
        for idx, device in enumerate(devices):
            ...
            # device_start_queue.put(serial)  # เพิ่มเข้า queue
        ...
        # process_device_start_queue()  # เริ่ม process คิว
    except Exception as e:
...
def reset_device(serial, on_stage=None):
    ...
    if main_configs.get('selected_connect_mode', '') == 'รีปกติ':
...
def capture_screen(device_serial: str, max_retries: int = 5):
    ...
    # สร้างชื่อไฟล์ปลอดภัย (10.0.0.34:5555 -> 10_0_0_34_5555)
...
@log_exception_to_json
def taptap_location(device_serial: str, x: int, y: int, is_ignore_x: bool = False):
    try:
        x = int(x)
        y = int(y)
        ...
        adb_run(['adb', '-s', device_serial, 'shell', 'input', 'tap', str(finalX), str(finalY)], timeout=20)
    except Exception as e:
        print(f'{device_serial}: taptap_location exception at ({x},{y}): {e}')
...
def detection(
    ...
    try:
        ...
        if mode == 'multiple':
            ...
            ui_queue.put(
                ('confidence', serial,
                f'{"FOUND ✅" if all_found else "NOT FOUND ❌"} Confidence: {max((r[3] for r in results.values()), default=0):.2f}')
            )
        ...
        if found:
            ...
            ui_queue.put(('confidence', serial, f'FOUND ✅ Confidence: {conf:.2f}'))
        else:
            ui_queue.put(('confidence', serial, f'NOT FOUND ❌ Confidence: {conf:.2f}'))
...
def file_transfer(serial, folder_name, accumulate_date=1, date=''):
    ...
    # sanitize folder_name to produce a safe prefix for the filename
    safe_folder_part = str(folder_name)
    # Normalize path separators and replace illegal filename chars with '_'
    safe_folder_part = safe_folder_part.replace('\\', '').replace('/', '')
    safe_folder_part = re.sub(r'[<>:\\"/\\|?*]', '', safe_folder_part)

    new_file_name = f'{accumulate_date}_[50]_{safe_folder_part.replace("_", "")}_{date}.dat'
    ...
    try:
        ...
        print(f'ADB command failed from {serial} -> {local_target}: {e}; stderr: {stderr}')
        return None
...
def extract_text_tesseract(
    ...
    try:
        ...
        logger.debug(f'{serial}: crop area -> {x1}, {y1}, {x2}, {y2}')
...
def launch_main_loop(serial, ui_queue: Queue, shared_data, shared_lock):
    ...
    def open_pes():
        ...
        (ADB ผ่าน TCP/IP ช้ากว่า loopback จึงต้อง retry + timeout ยาวขึ้น)"""
        max_retries = 3
        ...
        logger.error(f'Failed to open PES on {serial} after {max_retries} attempts')
        return False
    ...
    @log_exception_to_json
    def create_or_update_excel(linked_folder, username, password, current_file):
        ...
        try:
            ...
            new_row = DataFrame({
                'Username': [username],
                'Password': [password],
                'RealName': [current_file.split('_')[2] if len(current_file.split('_')) > 2 else current_file],
            })
            
            # ใช้ Lock เพื่อป้องกัน concurrent write
            with shared_lock:
                # 1. บันทึกลงไฟล์แต่ละชื่อ (เช่น Linked Playername.xlsx)
                file_path = os.path.join(excel_base_path, f'{linked_folder}.xlsx')
                ...
                total_file_path = os.path.join(excel_base_path, '0 Linked Total.xlsx')
    ...
    def handle_move_file(current_file, current_folder, username, password):    
        ...
        playername = name_list[2]

        current_file = f'Linked-{current_file}'
            
        new_file_name = current_file

        linked_folder = f'Linked {playername}'
        
        target_path = os.path.join(main_configs.get('re_reroll_file_path',''), current_folder, temp_current_file)
        move_file(linked_folder, target_path, new_file_name)

        time.sleep(2)
        
        # บันทึกข้อมูลลง Excel
        create_or_update_excel(linked_folder, username, password, current_file)
    ...
    def next_folder(serial, folder_list, index_folder, folder_path):
        ...
        if not sorted_files:
            try:
                if os.path.isdir(re_reroll_folder) and os.path.basename(re_reroll_folder).lower() != 'bin':
                    ...
                    print(f"{serial}: Skipping move for special or reserved folder {re_reroll_folder}")
            except Exception as e:
    ...
    def typing_keyevent(keycode):
        adb_run(['adb', '-s', str(serial), 'shell', 'input', 'keyevent', str(keycode)], timeout=5)
    ...
    def samak_new_account(current_file):
        ...
        clear_browser_cache(serial)
        time.sleep(2)
        
        # ✅ LOCK: ให้ device รอคิว ทีละตัว (ป้องกัน concurrent session)
        print(f'[LOGIN QUEUE] {serial}: Waiting for queue...')
        ui_queue.put(('substage', serial, '⏳ รอคิว login (ทีละตัว)...'))
        ...
        with login_queue_lock:
            ...
            ui_queue.put(('substage', serial, 'Login domain'))

            domain_email = 'pes@falsenineshop.com'
            domain_password = 'Pes_12345678'
            ...
            def loop_already_username():
                ...
                def set_already_exists():
                    nonlocal is_already_exists
                    is_already_exists = True

                filename = current_file.split('_')[2].replace(' ', '').replace('[', '').replace(']', '')

                if filename != 'NewSamak':
                    filename_splited = filename.rsplit('-')

                    if len(filename_splited) >= 2:
                        filename = ''.join(name[:3] for name in filename_splited)
                ...
                while True:
                    ui_queue.put(('substage', serial, 'สร้างบัญชีใหม่'))
                    wait_for(
                        serial=serial,
                        detection_type='text',
                        target_file='domain',
                    ...
                    username = f"{filename}_{to_base62(random.randint(0, 65535))}"
                    ...
                    ui_queue.put(('substage', serial, 'Check username already'))
                    wait_for(
                        serial=serial,
                        detection_type='text',
            ...
            ui_queue.put(('substage', serial, 'เช็คว่าสร้างเสร็จแล้ว'))
            wait_for(
                serial=serial,
                detection_type='text',
                target_file='account',
        ...
        # ✅ Release lock = device ถัดไปเข้าได้
        print(f'[LOGIN QUEUE] {serial}: <<<<<< RELEASED LOCK - Next device can proceed')
        ui_queue.put(('substage', serial, '✅ Login Complete'))
    ...
    def connect_mode():
        ...
        print(f'connect_mode result: {result}')
        if not result or len(result) != 4 or any(v is None for v in result):
            logger.error(f"{serial}: connect_mode aborted - no valid reroll file selected: {result}")
            ui_queue.put(('completed', serial, 'completed'))
            return

        current_file, current_folder, num_name, accumulat = result

        # ✅ Phase 4: Call samak_new_account dengan queue lock
        samak_result = samak_new_account(current_file)

        username, password = samak_result

        ui_queue.put(('substage', serial, 'หน้าหลัก pes'))
        wait_for(
            serial=serial,
            detection_type='text',
            target_file='contract', # Contracts
            text_action=lambda:[
                tap_location(serial, 811, 72)
            ],
            text_crop_area=(438, 497, 520, 520), # พื้นที่คำว่า Contracts
            extract_mode = 'name',
        )

        ui_queue.put(('substage', serial, 'หน้า Extras'))
        wait_for(
            serial=serial,
            detection_type='text',
            target_file='support', # Support
            text_action=lambda:[
                tap_location(serial, 750, 313)
            ],
            text_crop_area=(719, 303, 808, 338), # พื้นที่คำว่า support
            extract_mode = 'name',
        )

        ui_queue.put(('substage', serial, 'หน้า Link Data'))
        wait_for(
            serial=serial,
            detection_type='text',
            target_file='data', # Data
            text_action=lambda:[
                tap_location(serial, 175, 86)
            ],
            text_crop_area=(175, 86, 229, 114), # พื้นที่คำว่า Data
            extract_mode = 'name',
        )

        ui_queue.put(('substage', serial, 'Modal Link Data'))
        loop_confirm_wait_for(
            target_file='youcan', # Data
            text_action=lambda:[
                tap_location(serial, 244, 324)
            ],
            text_crop_area=(288, 171, 379, 200), # พื้นที่คำว่า Data
        )
        ...
        wait_for(
            serial=serial,
            detection_type='text',
            target_file='warn', # warning
            text_action=lambda:[
                handle_move_file(
                    current_file=current_file, 
                    current_folder=current_folder,
                    username=f"{username}@falsenineshop.com", 
                    password=password
                )
            ],
            text_crop_area=(433, 491, 527, 525), # 
            extract_mode = 'name',
        )
...
def poll_queues(app):
    ...
    for serial, q in device_queues.items():
        try:
            while True:
                try:
                        ...
                        if lbl and lbl.winfo_exists():
                            try:
                                ...
                                if isinstance(payload, str):
                                    ...
                                    lbl.configure(text=f'{port} : {payload}', text_color='red')
                                else:
                                    # ถ้า payload เป็น int (วินาที) ให้แสดง m:ss format
                                    minutes = int(payload) // 60
                                    seconds = int(payload) % 60
                                    lbl.configure(text=f'{port} : {minutes}:{seconds:02d}', text_color='white')
                            except Exception as e:
    ...
    for serial, p in list(device_procs.items()):
        ...
        if not p.is_alive():
            ...
            if exit_code is not None and exit_code != 0:
                logger.error(f"⚠️ ALERT: Process for device {serial} crashed! Exit code: {exit_code}")
                if exit_code == -9:
                ...
                if serial in device_queues:
                    safe_queue_put(device_queues[serial], 
                                 ('remaining', serial, f'❌ CRASHED (exit code: {exit_code})'),
...
if __name__ == '__main__':
    ...
    def on_select_mode(choice):
        ...
        selected_connect_mode.set(choice)  # อัปเดตค่าลงใน global variable
        load_main_config()
        main_configs['selected_connect_mode'] = choice
        save_main_config()
        load_main_config()

        if connect_button:
            colspan = 1 if choice == 'connect' or choice == 'ฟาร์ม' else 2
    ...
    def update_path_frame_visibility():
        if selected_connect_mode.get() == 'connect':
            path_frame.grid()
        else:
